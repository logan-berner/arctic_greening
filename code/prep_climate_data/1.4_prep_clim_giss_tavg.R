# COMPUTE ANNUAL SUMMER WARTH INDEX USING NASA GISS MONTHLY TEMPERATURE ANOMALIES
rm(list=ls())
require(ncdf4)
require(raster)
require(maptools)
...
anom.stk <- stack('gis_data/climate/giss/tavg/gistemp1200_ERSSTv5.nc', varname = 'tempanomaly') # anomalies from 1951-1980 climatology
plot(anom.stk[[1638]])
...
# output directories
mkdirs('A:/research/data/climate/giss/tavg/monthly')
mkdirs('gis_data/climate/giss/tavg/monthly')
mkdirs('gis_data/climate/giss/swi/monthly/')
mkdirs('gis_data/climate/giss/swi/annual/')


# CREATE CLIMATOLOGY (1951-1980) USING BERKELEY EARTH, CRU, and UDEL -------------------------------------------
climotology.yrs <- 1951:1980
...
# MONTHLY MEAN TEMPERATURE FROM 1850 - 2017 -----------------------------------------------
clust <- makeCluster(8)
...
foreach(i=1:n.ts) %dopar% {
    ...
    # grab baseline for specific month from ensemble
    tavg.aoi.laea <- subset(abs.stk, ts.months[i]) # these are already multiplied by 10

    # subset anom data to time period and scale
    ...
    # add baseline to anomalies
    r.aoi.laea <- tanom.aoi.laea + tavg.aoi.laea
    
    # write out
    r.aoi.laea <- round(r.aoi.laea)
    arctic.outname <- paste('gis_data/climate/giss/tavg/monthly/giss_tavg_', ts.year[i], '_', months[ts.months[i]],'_degCx10_50km_laea.tif', sep='')
...
swi.outdir <- paste('gis_data/climate/giss/swi/monthly', sep='')
mkdirs(swi.outdir)

tmp.files <- list.files('gis_data/climate/giss/tavg/monthly/', full.names = T)
...
swi.yrly.outdir <- 'gis_data/climate/giss/swi/annual/'
mkdirs(swi.yrly.outdir)

n.yrs <- length(yrs)
swi.monthly.files <- list.files('gis_data/climate/giss/swi/monthly/', full.names = T)
swi.annual.outnames <- paste(swi.yrly.outdir, 'giss_swi_', yrs, '_degCx10_50km_laea.tif', sep='') 
