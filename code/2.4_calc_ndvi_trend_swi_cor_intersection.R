...
for (i in 1:mc.reps){
  ...
  site.dt <- ndvi.swi.cor.dt[ndvi.trend.dt, on = c('period','site')]
  ...
  # STORE TO LIST
  cor.by.ndvi.trend.dt[, rep := i]
  outlist[[i]] <- cor.by.ndvi.trend.dt 
  
  print(i)
}


DT <- rbindlist(outlist)

smry.dt <- DT[, .(pcnt.med = median(pcnt.sites), pcnt.q025 = quantile(pcnt.sites, 0.025), pcnt.q975 = quantile(pcnt.sites, 0.975)) , by = c('period','trend.cat','cor.cat')]
fwrite(smry.dt, 'output/lsat_site_swi_cors/lsat_ndvi_trend_with_swi_cor_smry.csv')
